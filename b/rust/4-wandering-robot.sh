...
binary=kevan-tmp-file
source_code="/tmp/$binary.rs"

# Cat the source code to a temp file.
...
# Compile, only once. Tested and works for Google Kick Start.
if [[ ! -f "$binary" ]]; then
    rustc -O "$source_code" || exit 1
fi

# stdin is automatically redirected to the binary.
"./$binary"
